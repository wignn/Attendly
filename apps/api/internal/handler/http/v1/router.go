...
func RegisterRoutes(r chi.Router, h Handlers, tokenMaker *token.Maker, redisClient *redis.Client, userRepository domain.UserRepository) {
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			if h.Teacher != nil {
				r.With(middleware.RequireRole(domain.RoleSuperAdmin)).Get("/teachers", h.Teacher.List)
				r.With(middleware.RequireRole(domain.RoleSuperAdmin)).Post("/teachers", h.Teacher.Create)
				r.With(middleware.RequireRole(domain.RoleSuperAdmin)).Get("/teachers/{teacher_id}", h.Teacher.Get)
			...
			if h.ExportAudit != nil {
				...
				r.With(middleware.RequireRole(domain.RoleSuperAdmin, domain.RoleTeacher, domain.RoleHomeroomTeacher)).Get("/exports/{job_id}", h.ExportAudit.GetExportJob)
